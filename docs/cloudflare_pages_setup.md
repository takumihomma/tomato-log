...
| **Deploy command (デプロイコマンド)** | `npx wrangler deploy` | **【重要】** wrangler.jsonのassets指定に従いデプロイ |

> **💡 自動デプロイの仕組み:**
> プロジェクト直下の `wrangler.json` に `"assets": { "directory": "./dist" }` を定義しているため、`npx wrangler deploy` がビルド成果物 `./dist` 内のアセットを本番環境へアップロードします。
